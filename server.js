...
// Server ambil ulang link asli dari TikTok/IG lalu proxy bytes-nya ke client
// dengan header (Referer, User-Agent) yang benar, jadi gak kena blokir 403.
app.get('/api/download/stream', async (req, res) => {
  ...
  exec(cmd, { timeout: 20000 }, async (err, stdout) => {
    if (err) return res.status(502).send('Gagal ambil video.');
    ...
    try {
      ...
      const upstream = await fetch(directUrl, {
        headers: {
          ...
          'Referer': `https://${domain.replace('vt.', '').replace('vm.', '')}/`,
        },
      });
      ...
      res.status(502).send('Gagal mengambil file video dari server sumber.');
    }
  });
});
