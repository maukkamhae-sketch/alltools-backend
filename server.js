...
app.get('/api/pulsa/orders', requireAuth, (req, res) => {
  res.json({ orders: db.getOrdersForUser(req.user.id).filter(o => !['premium', 'bot'].includes(o.type)) });
});
...
app.get('/loading.webp', (req, res) => {
  const file = path.join(__dirname, 'loading.webp');
...
app.get('/api/admin/config', requireOwner, (req, res) => {
  res.json({ products: getRawProducts(), planPrices: getPlanPrices(), botPrices: getBotPrices(), botNumber: getBotNumber(), dana: getDanaNumber() });
});

app.put('/api/admin/config', requireOwner, (req, res) => {
  const { products, planPrices, botPrices, botNumber, dana } = req.body || {};
  ...
  res.json({ ok: true, products: getRawProducts(), planPrices: getPlanPrices(), botPrices: getBotPrices(), botNumber: getBotNumber(), dana: getDanaNumber() });
});
