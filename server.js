...
app.post('/api/manual/create', requireAuth, (req, res) => {
  ...
  if (type === 'premium') {
    ...
    if (!PLAN_PRICES[p]) return res.status(400).json({ error: 'Paket tidak dikenali.' });
    label = `AllTools Premium ${p}`;
    price = PLAN_PRICES[p];
    extra = { type: 'premium', plan: p };
  } else {
    const product = PULSA_PRODUCTS.find(x => x.id === productId);
    if (!product) return res.status(400).json({ error: 'Produk tidak dikenali.' });
    const phone = (phoneNumber || '').replace(/[^0-9]/g, '');
    if (!phone || phone.length < 8) return res.status(400).json({ error: 'Nomor HP tujuan tidak valid.' });
    label = product.label;
  ...
  res.json({ orderId, label, total, dana: PAYMENT_DANA_NUMBER });
});
...
async function handleTelegramUpdate(u) {
  ...
  if (order.status === 'paid' || order.status === 'failed') {
    return tgApi('answerCallbackQuery', { callback_query_id: cb.id, text: 'Pesanan ini sudah diproses.' });
  }
  let note;
  if (action === 'ok') {
    order.status = 'paid';
    ...
    if (order.type === 'premium') {
      ...
      note = `✅ Dikonfirmasi. Paket ${order.plan} sudah diaktifkan.`;
    } else {
      note = `✅ Dikonfirmasi. Tolong isikan ${order.productLabel} ke ${order.phoneNumber} ya.`;
    }
  } else {
    order.status = 'failed';
    db.saveOrder(order);
    note = '❌ Pesanan ditolak.';
  }
  await tgApi('answerCallbackQuery', { callback_query_id: cb.id, text: note.slice(0, 190) });
  ...
  await tgApi('sendMessage', { chat_id: cb.message.chat.id, text: `${note}\nOrder ID: ${order.orderId}` });
}
