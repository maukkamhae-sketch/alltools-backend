...
function getBlockedApps(deviceId) {
  return readDb().amDevices.find(d => d.id === deviceId)?.blockedApps || [];
}
function setBlockedApps(deviceId, packages) {
  const db = readDb();
  const dev = db.amDevices.find(d => d.id === deviceId);
  if (dev) { dev.blockedApps = packages; writeDb(db); }
}
